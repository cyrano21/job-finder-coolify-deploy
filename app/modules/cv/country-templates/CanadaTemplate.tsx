...
export default function CanadaTemplate({ cv }: { cv: CV }) {
  ...
  return (
    <div className="bg-white font-sans w-full min-h-full p-8">
      {/* Canadian Header with bilingual support */}
      <header className="border-b border-gray-300 pb-4 mb-6">
        <div className="flex justify-between items-start">
          ...
          <div className="text-right text-sm">
            <div className="font-medium">Contact Information / Information de contact</div>
      ...
      <div className="space-y-6">
        ...
        {summarySection && (summarySection.content as Summary).content && (
          <section>
            <h3 className="text-xl font-semibold mb-3 uppercase border-b-2 border-red-600 pb-1">
              {summarySection.title} / Profil professionnel
            </h3>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-6">
            ...
            {experienceSection && (experienceSection.content as Experience[]).length > 0 && (
              <section>
                <h3 className="text-xl font-semibold mb-3 uppercase border-b-2 border-red-600 pb-1">
                  {experienceSection.title} / Expérience de travail
                </h3>
                <div className="space-y-4">
                  {(experienceSection.content as Experience[]).map((experience) => (
                    <div key={experience.id} className="relative pl-4 before:content-[''] before:absolute before:left-0 before:top-2 before:w-2 before:h-2 before:bg-red-600 before:rounded-full">
                      <div className="flex justify-between mb-1">
                        <h4 className="font-semibold">{experience.title}</h4>
                        <span className="text-sm text-gray-600">
                          {experience.startDate} - {experience.current ? 'Present / Présent' : experience.endDate}
                        </span>
                      </div>
            ...
            {educationSection && (educationSection.content as Education[]).length > 0 && (
              <section>
                <h3 className="text-xl font-semibold mb-3 uppercase border-b-2 border-red-600 pb-1">
                  {educationSection.title} / Éducation
                </h3>
                <div className="space-y-4">
                  {(educationSection.content as Education[]).map((education) => (
                    <div key={education.id} className="relative pl-4 before:content-[''] before:absolute before:left-0 before:top-2 before:w-2 before:h-2 before:bg-red-600 before:rounded-full">
                      <div className="flex justify-between mb-1">
                        <h4 className="font-semibold">{education.degree}</h4>
                        <span className="text-sm text-gray-600">
                          {education.startDate} - {education.current ? 'Present / Présent' : education.endDate}
                        </span>
                      </div>
          ...
          <div className="space-y-6">
            ...
            {languagesSection && (languagesSection.content as Language[]).length > 0 && (
              <section>
                <h3 className="text-xl font-semibold mb-3 uppercase border-b-2 border-red-600 pb-1">
                  {languagesSection.title} / Langues
                </h3>
                <div className="space-y-2">
                  {(languagesSection.content as Language[]).map((language) => (
                    <div key={language.id}>
                      <div className="flex justify-between">
                        ...
                        <span className="text-sm text-gray-600">
                          {language.level === 'native' ? 'Native / Langue maternelle' : language.level}
                        </span>
                      </div>
                    </div>
                  ))}
      ...
      <div className="mt-8 text-center text-sm text-gray-500">
        <p>References available upon request / Références disponibles sur demande</p>
      </div>
    </div>
  )
}